...
import {
  deleteExpertPendingComplain,
  deleteExpertSolveComplain,
  getExpertPendingComplain,
  getExpertSolvedComplain,
...
const ExpertComplain = () => {
  const dispatch = useAppDispatch();


  const { expertPendingComplain, expertSolvedComplain } = useSelector(
  ...
  useEffect(() => {
    ...
    if (type == "pending") {
      status = 1;
      dispatch(getExpertPendingComplain(status));
    } else {
      status = 2;
      dispatch(getExpertSolvedComplain(status));
    }
  }, [dispatch, type]);
  ...
  const handleDelete = (id: any) => {
    ...
    data
      .then((logouts) => {
        ...
        if (yes && type == "pending") {
          dispatch(deleteExpertPendingComplain(id));
        } else {
          dispatch(deleteExpertSolveComplain(id));
        }
      })
      .catch((err) => console.log(err));
  };

  const expertPendingComplainTable = [
    {
      Header: "No",
    ...
    },
    {
      ...
      Cell: ({ row }: { row: ComaplainTable }) => (
        <span className="text-capitalize fw-bold">{row?.appointmentId ? row?.appointmentId : '-'}</span>
      ),
    },
    {
      Header: "Description",
      ...
      Cell: ({ row }) => (
        <span>
          <button
            ...
            onClick={() =>
              dispatch(openDialog({ type: "expertpendingcomplain", data: row }))
            }
          >
            <svg
              width="24"
  ...
  const expertSolvedComplainTable = [
    {
      Header: "No",
      Cell: ({ index }: { index: any }) => (
    ...
    },
    {
      ...
      Cell: ({ row }: { row: ComaplainTable }) => (
        <span className="text-capitalize fw-bold">
          {row?.expert?.name ? row?.expert?.name : "-"}
        </span>
      ),
    },
    {
  ...
  return (
    <>
      <>
        ...
        <div>
          {dialogueType == "expertpendingcomplain" && <UserComplainDialog />}

          {type === "pending" && (
            <>
              <Table
                type={"client"}
                data={expertPendingComplain}
                mapData={expertPendingComplainTable}
                PerPage={rowsPerPage}
                Page={page}
              />
              <Pagination
                ...
                totalData={expertPendingComplain?.length}
              />
            </>
          )}
          ...
          {type === "solved" && (
            <>
              <Table
                type={"client"}
                data={expertSolvedComplain}
                mapData={expertSolvedComplainTable}
                PerPage={rowsPerPage}
                Page={page}
              />
              <Pagination
                ...
                onPageChange={handleChangePage}
                onRowsPerPageChange={handleChangeRowsPerPage}
                totalData={expertSolvedComplain?.length}
              />
            </>
          )}
        </div>
