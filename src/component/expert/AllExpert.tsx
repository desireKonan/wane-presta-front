...
import { blockExpert, deleteExpert, getAllExpert } from "@/store/expertSlice";
...
import NotificationDialogue from "../user/NotificationDialogue";
...
const AllExpert = () => {
  ...
  const { expert, total } = useSelector((state: RootStore) => state.expert);
  ...
  const handleNotify = (id: any) => {
    dispatch(openDialog({ type: "notification", data: { id, type: "expert" } }));
  };
  ...
  const expertTable = [
    ...
    {
      Header: "Image",
      Cell: ({ row, index }: { row: ExpertData; index: number }) => (
        <div className="userProfile">
          <img
            src={baseURL + row?.profileImage}
            width="70px"
            height="70px"
            alt={`Expert ${(page - 1) * rowsPerPage + index + 1}`}
          />
        </div>
      ),
    },
    {
      Header: "Name",
      Cell: ({ row }: { row: ExpertData }) => (
        <span className="text-capitalize fw-bold cursor">{row?.name}</span>
      ),
    },
    {
      Header: "UniqueId",
      Cell: ({ row }: { row: ExpertData }) => <span>{row?.uniqueId}</span>,
    },
    {
      Header: "Mobile No",
      Cell: ({ row }: { row: ExpertData }) => (
    ...
    },
    {
      Header: "Experience",
      Cell: ({ row }: { row: ExpertData }) => (
        <span>{row?.experience ? row?.experience : "-"}</span>
      ),
    },

    {
      Header: "Country",
      Cell: ({ row }: { row: ExpertData }) => <span>{row?.country}</span>,
    },

    {
      Header: "Total Appointments",
      Cell: ({ row }: { row: ExpertData }) => (
        <span>{row?.totalAppointments}</span>
      ),
    },
  ...
  const handleOpenBookings = (id: any) => {
    router.push({
      pathname: "/provider/providerbooking",
      query: { id: id },
    });
  };
  ...
  const handleEarning = (id: any) => {
    router.push({
      pathname: "/provider/providerEarning",
      query: { id: id },
    });
  };
